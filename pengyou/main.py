...
if user_input:
    with st.chat_message("user"): # 사용자 메시지 컨테이너 생성
        st.write(user_input) # 사용자 메시지 표시
    
    st.session_state.messages.append({"role": "user", "content": user_input}) # 사용자 메시지를 session_state에 추가
    ...
    completion = client.chat.completions.create( # OpenAI API 클라이언트를 사용하여 챗봇 응답 생성
            ...
            ] + st.session_state.messages  # 이전 대화 기록을 포함
        )
    bot_response = completion.choices[0].message.content # 챗봇 응답 추출
    
    with st.chat_message("assistant"): # 챗봇 메시지 컨테이너 생성
        st.write(bot_response) # 챗봇 메시지 표시
    
    st.session_state.messages.append({"role": "assistant", "content": bot_response}) # 챗봇 응답을 session_state에 추가